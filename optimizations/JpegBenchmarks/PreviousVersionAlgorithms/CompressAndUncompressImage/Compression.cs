...
using JpegBenchmarks.PreviousVersionAlgorithms.HuffmanCoding;
using JpegBenchmarks.PreviousVersionAlgorithms.LoadAndSave;
using JpegBenchmarks.PreviousVersionAlgorithms.MatrixAndPixels;

namespace JpegBenchmarks.PreviousVersionAlgorithms.CompressAndUncompressImage
{
			...
			for(var y = 0; y < matrix.Height; y += DCTSize)
			{
				for(var x = 0; x < matrix.Width; x += DCTSize)
				{
					foreach (var selector in new Func<Pixel, double>[] {p => p.Y, p => p.Cb, p => p.Cr})
					{
						...
						var channelFreqs = DCTPreviousVersion.DCT2D(subMatrix);
				...
				for (var y = 0; y < image.Height; y += DCTSize)
				{
					for (var x = 0; x < image.Width; x += DCTSize)
					{
						...
						foreach (var channel in new []{_y, cb, cr})
						{
							...
							DCTPreviousVersion.IDCT2D(channelFreqs, channel);
